...
def plot_epipolar_lines(image1, image2, uncalibrated_1, uncalibrated_2, E, K, plot=True):
    ...
    """ YOUR CODE HERE
    """
    E_ = np.linalg.inv(K).T@(E@ np.linalg.inv(K))
    epipolar_lines_in_1 = []
    epipolar_lines_in_2 = []

    print(np.shape(uncalibrated_1))
    for i in range(np.shape(uncalibrated_1)[1]):
      epipolar_lines_in_1.append(E_.T @ uncalibrated_2[:,i])
    
    for i in range(np.shape(uncalibrated_2)[1]):
      epipolar_lines_in_2.append(E_ @ uncalibrated_1[:,i])
